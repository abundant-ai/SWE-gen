...
from . import PRToHarborPipeline, TrivialPRError, MissingIssueError
...
def _check_linked_issues(
    ...
    if require_issue:
        if not linked_issues:
            console.print(Panel(
                Text(f"PR #{pr} has no linked issue. Use --no-require-issue to generate task from PR body/title instead.", style="yellow"),
                title="[yellow]Skipped (No Linked Issue)[/yellow]",
                border_style="yellow",
            ))
            raise MissingIssueError(f"PR #{pr}: No linked issue found (use --no-require-issue to skip this check)")
        else:
        ...
        if linked_issues:
            ...
            console.print("[yellow]No linked issue found - using PR body/title for instructions[/yellow]")
...
def _build_validation_table(results_rows: list[list[str]]) -> Table | None:
    ...
    vt = Table(title="Validation Results", title_style="bold cyan", header_style="bold", show_lines=False)
    vt.add_column("Phase")
    vt.add_column("Expected")
    vt.add_column("Actual")
    vt.add_column("Match?")
    for phase, expected, actual, match in results_rows:
        vt.add_row(phase, expected, actual, Text(match, style=("green" if match == "Yes" else "red")))
    return vt
...
def _handle_validation_failure(
    ...
    if cc_validation_failed and not harbor_validation_failed and harbor_actually_ran:
        console.print()
        console.print(Panel(
            Text("⚠ CC validation failed, but Harbor validation passed.\nThis is acceptable - Harbor is the authoritative test environment.", style="yellow bold"),
    ...
    validation_failed = harbor_validation_failed or (cc_validation_failed and not harbor_actually_ran)
    ...
    if validation_failed:
        ...
        if cc_validation_failed and not harbor_actually_ran:
            ...
            console.print(Panel(
                Text("CC validation failed and Harbor validation was skipped.\nThe task cannot be verified. Run Harbor validation manually or re-run with --validate.", style="red bold"),
...
def _display_summary_panel(
    ...
    try:
        ...
        if not test_files:
            test_files = list((task_dir / "tests").glob("*.js")) + list((task_dir / "tests").glob("*.ts"))
...
def _display_logs_panel(
    ...
    if network_isolated:
        logs.add_row("Harbor nop-no-network job", Text(harbor_nop_no_net_job_dir or "-", overflow="fold"))
        logs.add_row("Harbor oracle-no-network job", Text(harbor_oracle_no_net_job_dir or "-", overflow="fold"))
...
def _display_next_steps_panel(
    ...
    steps.add_row("6.", f"Create a pull request including the new task under {harbor_root / task_id}")
...
def run_reversal(config: ReversalConfig) -> None:
    ...
    try:
        ...
        try:
            ...
            console.print("[dim]  → Cloning/updating repo cache (may take a minute for first clone)...[/dim]")
            ...
            console.print("[dim]  → Generating universal skeleton (includes LLM evaluation)...[/dim]")
            with console.status("Evaluating PR & writing skeleton...", spinner="dots"):
                task_dir, _, extracted_test_files, task_reference = pipeline.generate_task_universal(
            ...
            console.print(f"[green]✓ Skeleton generated in {skeleton_secs:.1f}s → {task_dir}[/green]")
            console.print(f"  [dim]Test files: {len(extracted_test_files)}[/dim]")
            
            # Step 2: Run CC "make it work" session
            console.print()
            if task_reference:
                console.print(Rule(Text(f"Claude Code: Copy & Adapt from PR #{task_reference.pr_number}", style="bold magenta")))
                console.print(f"[dim]Reference: {task_reference.task_id} | Timeout: {config.cc_timeout}s | Verbose: {str(verbose).lower()}[/dim]")
            else:
                console.print(Rule(Text("Claude Code: Make It Work", style="bold magenta")))
                console.print(f"[dim]Timeout: {config.cc_timeout}s | Verbose: {str(verbose).lower()}[/dim]")
            console.print()
            ...
            if cc_result and cc_result.success:
                ...
                console.print()
                console.print(f"[yellow]⚠ CC session completed in {gen_secs:.1f}s (validation incomplete)[/yellow]")
                if cc_result.error_message:
                    console.print(f"  [red]Error: {cc_result.error_message}[/red]")
            else:
                console.print(f"[green]✓ Skeleton generated in {gen_secs:.1f}s → {task_dir}[/green]")
        ...
        if harbor_do:
            ...
            harbor_jobs = (config.state_dir / "harbor-jobs" if isinstance(config.state_dir, Path) else Path(".state") / "harbor-jobs")
        ...
        if harbor_do and config.network_isolated:
            ...
            with network_isolation(task_dir):
                harbor_nop_no_net_job = _run_harbor_with_status(task_id, harbor_root, harbor_jobs, console, "nop", environment=config.environment)
                harbor_oracle_no_net_job = _run_harbor_with_status(task_id, harbor_root, harbor_jobs, console, "oracle", environment=config.environment)
            ...
            harbor_nop_no_net_job_dir = str(harbor_nop_no_net_job.parent) if harbor_nop_no_net_job else None
            harbor_oracle_no_net_job_dir = str(harbor_oracle_no_net_job.parent) if harbor_oracle_no_net_job else None
            results_rows.append(["Harbor nop-no-network", "reward=0", f"reward={reward_nop_no_net}" if reward_nop_no_net is not None else "reward=unknown", "Yes" if reward_nop_no_net == 0 else "No"])
            results_rows.append(["Harbor oracle-no-network", "reward=1", f"reward={reward_oracle_no_net}" if reward_oracle_no_net is not None else "reward=unknown", "Yes" if reward_oracle_no_net == 1 else "No"])

        # Display validation results and check for failures
        harbor_validation_failed, cc_validation_failed = _display_validation_results(console, results_rows)
        ...
        _handle_validation_failure(console, harbor_validation_failed, cc_validation_failed, harbor_actually_ran)

        # Save state record (non-fatal if fails)
        _save_state_record(state_dir, state_file, repo_key, pipeline.repo, config.pr, task_id, task_dir)

        # Display final panels
        _display_summary_panel(console, pipeline.repo, config.pr, task_id, task_dir, gen_log_path, validation_table)
        _display_logs_panel(
            console, gen_log_path,
            harbor_nop_job_dir, harbor_oracle_job_dir,
            config.network_isolated,
            harbor_nop_no_net_job_dir, harbor_oracle_no_net_job_dir,
        )
...
def _run_harbor_with_status(task_id: str, harbor_root: Path, harbor_jobs_parent: Path, console: Console, phase: str, delete_after: bool = True, environment: EnvironmentType = EnvironmentType.DOCKER) -> Path | None:
